...
import gradio as gr
from utils import OpenAI, QdrantClient, get_context_from_qdrant, build_messages, \
                  filter_relevant_sources, add_pdf_to_qdrant_index, rebuild_doc_map, \
                  delete_pdf_from_qdrant,move_pdf_files,remove_temp_gradio_file
import markdown
import pandas as pd
...
def delete_pdf_gradio(selected_pdf, qdrant, doc_map=doc_map):
    ...
    status = delete_pdf_from_qdrant(selected_pdf, qdrant,doc_map=doc_map)  # Assumes this handles the Qdrant deletion
    status = remove_temp_gradio_file(selected_pdf)  # remove do /tmp/gradio
    updated_list = get_pdf_list()

    return (
        status,
        gr.update(choices=updated_list, value=None)
    )
...
def chat_with_context(user_input, history):
    ...
    unique_sources = set()
    relevant_sources = filter_relevant_sources(assistant_reply, results, openai, threshold=0.80)

    if len(relevant_sources) > 0:
        unique_sources = set(relevant_sources)
        sources_text = "<br>".join(f"[Fonte: {s}, Página {p}]" for s, p in sorted(unique_sources))
    else:
        sources_text = ""
...
""") as demo:
    with gr.Tabs():
        with gr.TabItem("Chat"):
            ...
            """)
            gr.Image(value="images/da-vinci.png", width=50,height=50, show_label=False, container=False)
            ...
            user_input.submit(chat_with_context, inputs=[user_input, state], outputs=[user_input, state, chat_html])
        ...
        with gr.Tab("📄 Gerenciar arquivos"):
            with gr.Row():
                ...
                with gr.Column(elem_id="delete_column", scale=1):
                    ...
                    delete_dropdown = gr.Dropdown(choices=[], label="Escolha o PDF para remover")
